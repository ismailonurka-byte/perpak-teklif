...
export function ToastContainer() {
    ...
    return (_jsx("div", { className: "fixed bottom-4 right-4 z-[100] space-y-2 max-w-sm", children: list.map((t) => {
            ...
            const color = t.tip === "ok" ? "border-emerald-500 bg-white text-emerald-800"
                : t.tip === "err" ? "border-rose-500 bg-white text-rose-800"
                    : "border-blue-500 bg-white text-blue-800";
            return (_jsxs("div", { className: `flex items-start gap-2.5 rounded-xl border-l-4 px-4 py-3 shadow-elevated ring-1 ring-slate-200/60 animate-slide-up ${color}`, children: [_jsx(Icon, { size: 18, className: "mt-0.5 shrink-0" }), _jsx("div", { className: "flex-1 text-sm", children: t.mesaj }), _jsx("button", { onClick: () => remove(t.id), className: "text-current opacity-50 hover:opacity-100", children: _jsx(X, { size: 14 }) })] }, t.id));
        }) }));
}
